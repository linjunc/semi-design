...
class Sider extends React.PureComponent<SiderProps> {
    ...
    render() {
        ...
        return (
            <aside className={classString} role={this.props.role} aria-label={this.props['aria-label']} style={style} {...getDataAttr(others)}>
                <div className={`${prefixCls}-sider-children`} aria-label={'Content of Sider '}>
                    {children}
                </div>
            </aside>
        );
